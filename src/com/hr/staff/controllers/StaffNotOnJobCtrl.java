...
public class StaffNotOnJobCtrl extends BaseCtrl {
    ...
    public void list() {
        ...
        try {
            String select = "SELECT h_staff_log.id id, h_store.id store_id, h_store. NAME store_name, h_staff_log. NAME name, ( CASE h_staff_log.gender WHEN 1 THEN '男' ELSE '女' END ) gender, h_staff_log.phone phone, j.job job, REPLACE (k.kind, ',', '/') kind, CONCAT( h_staff_log.hour_wage, '/', h_staff_log.month_wage ) wage, w.type type, s.status_text status_text,h_staff_log.create_time datetime";
