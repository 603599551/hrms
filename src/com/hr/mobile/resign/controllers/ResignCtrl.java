...
//        staffId="2ab92020caa045c39e820e916413ad47";
        ...
        try {
            //参数为店长id
            String sql = "SELECT n.id id, upper(LEFT (s.pinyin, 1)) firstname, s. NAME name, s.job job, n.create_time time, n.content reason, ( CASE r. STATUS WHEN '0' THEN '0' ELSE '1' END ) ishandle, ( CASE r. STATUS WHEN '1' THEN '1' else '0' END ) isagree, ( SELECT count(*) FROM h_notice, h_resign WHERE h_notice.fid = h_resign.id AND h_notice.receiver_id = ? AND h_notice.type = 'resign' AND h_resign. STATUS = '0' ) untreated FROM h_staff s, h_notice n, h_resign r WHERE s.id = n.sender_id AND n.fid = r.id AND n.receiver_id = ? AND n.type = 'resign'";
            List<Record> applyList = Db.find(sql, staffId,staffId);

            String sqlAgree="SELECT n.id id, upper(LEFT (s.pinyin, 1)) firstname, s. NAME name, s.job job, n.create_time time, n.content reason, ( CASE r. STATUS WHEN '0' THEN '0' ELSE '1' END ) ishandle, ( CASE r. STATUS WHEN '1' THEN '1' else '0' END ) isagree, ( SELECT count(*) FROM h_notice, h_resign WHERE h_notice.fid = h_resign.id AND h_notice.receiver_id = ? AND h_notice.type = 'resign' AND h_resign. STATUS = '0' ) untreated FROM h_staff_log s, h_notice n, h_resign r WHERE s.staff_id = n.sender_id AND n.fid = r.id AND n.receiver_id = ? AND n.type = 'resign'";
