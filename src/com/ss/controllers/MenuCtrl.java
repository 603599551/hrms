    ...
    public void index() {
        ...
        try {
            if ("admin".equals(usu.getUsername())) {
                List<Record> list = Db.find("select * from menu order by sort");
                for (Record r : list) {
                ...
                List<Record> list = Db.find("select m.* from menu m,author_job_menu ajm where m.ID=ajm.menu_id and ajm.job_id=? and ajm.access='1' order by sort", jobId);
                for (Record r : list) {
                    r.set("title", r.get("name"));
                    r.remove("name");
                    r.set("link", r.get("url"));
