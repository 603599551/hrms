...
public class OutWarehouseOrderCtrl extends BaseCtrl{
    ...
    public void queryList(){
        ...
        String sql="select a.id, a.order_number, (select name from store where store.id = a.store_id) as store_text,(select store_color from store where store.id = a.store_id) as store_color, (select name from warehouse where warehouse.id = a.warehouse_id) as warehourse_text, a.out_time, a.status, case a.status when '10' then '新建' when '20' then '保存' when '30' then '出库' when '40' then '完成' end as status_text, (select d.status_color status_color from dictionary d where d.parent_id='1' and d.value=a.status) status_color, b.want_date, b.arrive_date,IFNULL((select sort from print_details x where x.order_id = b.id and x.order_number=a.order_number order by x.sort desc limit 1,1),0) as print_time,a.warehouse_id from warehouse_out_order a inner join store_order b on a.store_order_id = b.id";
        try{
