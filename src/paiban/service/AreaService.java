...
public class AreaService extends BaseService {
    ...
    public void edit(String id, String[] workers, String date, UserSessionUtil usu){
        Record areaStaff = Db.findFirst("select has.*, ha.id aid, ha.name aname from h_area_staff has, h_area ha where has.area_id=ha.id and has.area_id=?", id);
        ...
        for(String w : workers){
            ...
            r.set("staff_id", w);
            r.set("area_name", areaStaff.get("aname"));
            r.set("area_id", areaStaff.get("aid"));
            r.set("date", date);
